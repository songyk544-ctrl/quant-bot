...
#                 if output2:
#                     # 최근 20일 (약 1달) 누적 순매수 수량 합산 (리얼 데이터 매핑!)
#                     for daily in output2[:20]: 
#                         foreign_vol_sum += int(daily.get('frgn_ntby_qty', 0)) # 외국인
#                         pension_vol_sum += int(daily.get('pnsn_fund_ntby_qty', 0)) # 찐 연기금
#                         trust_pef_vol_sum += (int(daily.get('ivtr_ntby_qty', 0)) + int(daily.get('pef_ntby_qty', 0))) # 찐 투신+사모펀드
...
#             # 실전 AI 스코어링 (외인강도*5 + 연기금강도*10 + 투신사모*5) 기본 50점 세팅
...
st.set_page_config(page_title="수급 엔진 정밀 진단")
st.title("🚑 한투 수급 API 1초 진단기")
...
if st.button("🚀 삼성전자 리얼 수급 찔러보기"):
    with st.spinner("한투 서버 통신 중..."):
        ...
        headers = {
            ...
            "tr_id": "FHKST01010900", # ★ 범인이었던 TR_ID를 올바르게 수정!
            "custtype": "P"
        }
        url = f"{URL_BASE}/uapi/domestic-stock/v1/quotations/inquire-investor"
        ...
        st.json(res.json()) # 데이터가 output에 있는지 output2에 있는지 적나라하게 확인합니다.
