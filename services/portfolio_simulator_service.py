...
def build_portfolio_summary(perf, open_positions, closed_trades, initial_cash=5_000_000):
    daily = perf.copy() if perf is not None else pd.DataFrame()
    positions = open_positions.copy() if open_positions is not None else pd.DataFrame()
    closed = closed_trades.copy() if closed_trades is not None else pd.DataFrame()
    if daily.empty:
        metrics = compute_trade_quality_metrics(closed)
        return {
    ...
    last = daily.iloc[-1]
    final_equity = float(pd.to_numeric(last.get("평가금액", initial_cash), errors="coerce") or initial_cash)
    cash = float(pd.to_numeric(last.get("현금", 0.0), errors="coerce") or 0.0)
    invested_value = float(pd.to_numeric(last.get("투자금액", 0.0), errors="coerce") or 0.0)
    total_return = float(pd.to_numeric(last.get("수익률(%)", 0.0), errors="coerce") or 0.0)
    equity_curve = pd.to_numeric(daily.get("평가금액", pd.Series(dtype=float)), errors="coerce").dropna()
    ...
    return {
        ...
        "cash": cash,
        "cash_ratio": (cash / final_equity) if final_equity else 0.0,
...
def simulate_portfolio(
    ...
    daily, open_positions, closed_trades = build_capital_limited_swing_sim(
        candidate_trades,
        price_history,
    ...
    summary = build_portfolio_summary(daily, open_positions, closed_trades, initial_cash=initial_cash)
    return {
